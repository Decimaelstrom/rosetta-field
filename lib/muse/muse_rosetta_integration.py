...
Testing Muse agents using actual Rosetta-Field functions with A2A protocols
"""
...
# Add paths for Rosetta-Field imports
...
class RosettaEnabledAgent(LightweightA2AAgent):
    """Agent enhanced with Rosetta-Field function calls"""
    ...
    def __init__(self, name: str, model: str = "gemma3:1b"):
        ...
        print(f"🌹 {name} enhanced with {len(self.rosetta_functions)} Rosetta-Field functions")
    
    def load_rosetta_functions(self):
        """Load available Rosetta-Field functions"""
        try:
    ...
    def call_rosetta_function(self, function_name: str, **kwargs) -> Dict:
        """Call a Rosetta-Field function with A2A session context"""
...
def test_muse_rosetta_integration():
    """Test Muse agents using Rosetta-Field functions"""
    ...
    if orchestrator.rosetta_functions or presenter.rosetta_functions:
        ...
        print("✅ Agents can call Rosetta-Field functions via A2A protocols")
    else:
        print("⚠️  Integration limited - check Rosetta-Field imports")
